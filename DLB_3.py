...
for j in range(0,1):
	for t in range(T):
		...
		if p >= 0 and p < P[0]:
			...
			buffers = np.zeros(len(testPredict))
			buffers[:] = np.nan
			...
			if df_1.values[0,0] > threshold:
				...
				print 'Signal: %.0d' % (t), df_1.values[0,0]
				print 'Anomalous'
			else:
...
np.savetxt('Anomalies_d1', Anomalies_1)
np.savetxt('Non_anomalies_d1', Non_anomalies_1)    
np.savetxt('Predictions_d1', Predictions)
np.savetxt('dataset_d1', dataset)
np.savetxt('History_train_d1', History_train)
np.savetxt('History_validate_d1', History_validate)
